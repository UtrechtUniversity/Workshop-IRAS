...
make_batch_file <- function (node, walltime) {
  
  # Write the batch commands to this file
  #
  if (!dir.exists("batch")) dir.create("batch")
  batch_script <- file.path("batch", sprintf("batch2-%d.sh", node))
  ...
  writeLines("#SBATCH --mail-user=<e-mail address>", fbatch)
  ...
  writeLines("mkdir ./output", fbatch)
    ...
    #
  command    <- sprintf("Rscript ./R/digits_svm_bt_node.R -N %d &", node)
...
for (node in 1:N) {
  ...
  #
if (!dir.exists("batch")) dir.create("batch")
submit_script <- file.path("batch", "submit_svm2.sh")
...
for ( node in 1:N) {
  submit_cmd <- sprintf("sbatch --reservation=iraspd ./batch/batch2-%d.sh", node)
