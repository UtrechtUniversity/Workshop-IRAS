...
opt <- getopt::getopt(
  matrix(
    c('nodes',      'N', 1, "numeric",
      'walltime',   'W', 1, "numeric",
      'completion', 'T', 1, "numeric"
    ), 
    byrow=TRUE, ncol=4))
...
if (( is.null(opt$nodes) && is.null(opt$walltime)) ||
  ...
  print("Either nodes (-N) XOR walltime (-W) must be specified")
  quit(status = 1)
}

if (is.null(opt$completion)) {
  print("Completion time of task must be specified")
  quit(status = 1)
}
